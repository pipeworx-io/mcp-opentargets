...
`tools/list` at `https://gateway.pipeworx.io/opentargets/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/opentargets_target \
  -H 'Content-Type: application/json' \
  -d '{"ensembl_id":"ENSG00000141510"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/opentargets_target`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
